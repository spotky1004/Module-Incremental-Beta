 ...
 * @returns {T} 
 */
function typeEqualize(target, source) {
  if (source === null) {
    target = null;
  } else if (source instanceof Decimal) {
    if (
      !(
        target instanceof Decimal ||
        ["string", "number"].includes(typeof target)
      )
    ) target = source;
    target = Decimal(target ?? source);
...
window.typeEqualize = typeEqualize;

export default typeEqualize;
